...
if(item){
  ...
  {item.colors?.map((color, index) => (
    <div key={index} onClick={() => pickColor(color)} className={`rounded-full w-8 h-8 border-2 border-black mx-2 cursor-pointer ${color} ${pickedColor === color ? 'border-4 border-blue-500': ''}`}>
    </div>
  ))}
</div>
